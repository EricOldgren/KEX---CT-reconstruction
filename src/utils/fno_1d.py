...
class SpectralConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, max_mode, dtype=None):
        ...
        self.weights = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, self.modes, dtype=self.cdtype))
    ...
    def forward(self, x):
        ...
        out_ft = torch.zeros(batchsize, self.out_channels, x.size(-1)//2 + 1,  device=x.device, dtype=self.cdtype)
...
class FNO1d(nn.Module):
    def __init__(self, modes, in_channels, out_channels, layer_widths=None, verbose=False, dtype=None):
        ...
        if layer_widths is None:
            ...
            self.n_layers = len(layer_widths)+1 #Modified
        ...
        #Modified
        self.inp = SpectralConv1d(in_channels, self.layer_widths[0], modes, dtype=self.dtype) #nn.Linear(in_channels, self.layer_widths[0], dtype=self.dtype)        
