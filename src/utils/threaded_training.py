...
def train_thread(model: ModelBase, dataloader: DataLoader, n_epochs: int = 100, lr=0.01, use_reg = False, regularisation_lambda: float = 1e-3, display_loss: bool = False):
...
def multi_threaded_training(model: torch.nn.Module, dataset: Dataset, n_epochs=100, batch_size=32, lr=0.01, use_reg = False, regularisation_lambda=1e-3, num_threads=8):
    """
    ...
    for rank in range(num_threads):
        ...
        p = mp.Process(target=train_thread, args=(model, dataloader, n_epochs, lr, use_reg, regularisation_lambda, rank==0))
        p.start()
        processes.append(p)
