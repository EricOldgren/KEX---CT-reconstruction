...
angle_partition = odl.uniform_partition(0, np.pi*0.8, 100)
...
def update_display(test_sample, kernel_freq):
    ...
    test_loss=loss_fn(filtered_back_projected-test_sample)
...
if __name__ == '__main__':
    ...
    #pol_coeff=torch.arange(10).to(device)
    #pol_coeff=torch.randn(pol_coeff.shape).to(device)

    kernel_freq = torch.arange(np.ceil(0.5 + 0.5*t_size)).to(device)

    n_coeff=2
    x=torch.linspace(0,kernel_freq.shape[0],kernel_freq.shape[0])
    pol_list=[]
    for i in range(n_coeff):
        pol_list.append(i)
    pol = torch.tensor(pol_list)
    xx = x.unsqueeze(-1).pow(pol).to(device)

    model = torch.nn.Sequential(
    torch.nn.Linear(n_coeff, 1),
    ...
    #optimizer = torch.optim.Adam([model], lr=0.003)
