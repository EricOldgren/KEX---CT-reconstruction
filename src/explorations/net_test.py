...
EPOPCHS =      [30] #, 100, 100,  100, 60]
TRAINED = {}

scale_factor = 10 #output is zero all the time, a large scale factor will increase the penalty of that output

if __name__ == '__main__':

    for ar, n_epochs in zip(ANGLE_RATIOS, EPOPCHS):
        geometry = Geometry(ar, 200, 100, reco_shape=(128, 128))
        train_sinos, train_y, test_sinos, test_y = setup(geometry, num_to_generate=400, train_ratio=0.99, use_realistic=False, data_path="data/kits_phantoms_256.pt")
        #model = FBPNet(geometry, use_smooth_filters=True, n_fbps=4)
        ...
        multi_threaded_training(model, dataset, n_epochs=n_epochs, batch_size=20, lr=0.003, regularisation_lambda=0.00, num_threads=10)
