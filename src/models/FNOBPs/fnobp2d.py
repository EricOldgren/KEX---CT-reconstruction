...
class FNO_BP2D(FBPModelBase):
    ...
    def __init__(self, geometry: FBPGeometryBase, ar: float, hidden_layers: List[int], modes_y = 50, modes_x=50, M = 100, K = 100, PolynomialFamilyKey: int = Chebyshev.key, l2_reg = 0.01, use_base_filter=False):
        super().__init__()
        self._init_args = (ar, hidden_layers, modes_y, modes_x, M, K, PolynomialFamilyKey, l2_reg, use_base_filter)
    ...
    def get_extrapolated_filtered_sinos(self, sinos: torch.Tensor, known_angles: torch.Tensor, angles_out = None):
        ...
        return self.fno2d(sinos[:, None])[:, 0] + self.geometry.inverse_fourier_transform(self.geometry.ram_lak_filter()*self.geometry.fourier_transform(sinos))
