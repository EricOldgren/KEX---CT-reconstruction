...
from geometries import HTC2022_GEOMETRY, DEVICE, DTYPE, enforce_moment_constraints

from models.SerieBPs.fnoencoder import FNO_Encoder
from models.modelbase import  save_model_checkpoint
from models.discriminators.dcnn import DCNN, concat_pred_inp, Informed_DCNN, save_gan, load_gan
...
G = FNO_Encoder(geometry, ar, M, K, [80, 80], [80, 80], Legendre.key)
...
for it in range(n_iters):
    ...
    for epoch in range(epoch_f(it)):
        ...
        for phantom_batch, sino_batch in dataloeder:
            ...
            projected_coefficients = enforce_moment_constraints(geometry.series_expand(sino_batch, Legendre, M, K))
    ...
    if (it+1) % 10 == 0:
        save_gan(G, D, optimizer, loss_s, ar, f"gan_iter{it+1}_loss_{loss_s.item()}.pt")
