...
from geometries.parallel_geometry import ParallelGeometry, DEVICE
...
class SinoMoments:
    ...
    def __init__(self, full_geometry: ParallelGeometry, n_moments = 12) -> None:
        ...
        phis = torch.linspace(0.0, torch.pi, steps=full_geometry.Np, device=DEVICE)
        self.natural_basis = []
        ...
        for ni in range(n_moments):
            ...
            bi = torch.zeros(ni+1, full_geometry.Np, device=DEVICE)
    ...
    def reduce_space(self, basis: torch.Tensor, ni: int, maintain_on = True):
        ...
        assert phi_size == self.geometry.Np and t_size == self.geometry.Nt
...
#     n = basis.shape[0]
#     trunced = basis[:, :g.Np, :].reshape(n, -1)
#     # trunced = trunced / torch.sum(trunced**2, dim=-1)[:, None]
#     proj_B = trunced @ trunced.mH
#     proj_basis = get_ON(basis.reshape(n, -1), B = proj_B, degenerate=True).reshape(-1, ext_g.Np, ext_g.Nt)
#     print("Projectin basis dimension is ", proj_basis.shape[0])

#     padding = torch.zeros(test_sinos.shape[0], ext_g.Np-g.Np, g.Nt)
